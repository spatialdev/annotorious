...
annotorious.mediatypes.image.ImageAnnotator = function (item, opt_popup) {
  ...
  this._eventBroker.addHandler(annotorious.events.EventType.SELECTION_CANCELED, function (args) {
    if (annotorious.events.ui.hasMouse)
      goog.style.showElement(self._editCanvas, false);
    if (!args || !args.e.shiftKey) self._currentSelector.stopSelection();
    //else goog.style.showElement(self._editCanvas, true);
  });
}
 ...
 */
annotorious.mediatypes.image.ImageAnnotator.prototype.stopSelection = function (opt_original_annotation, keepShape) {
  if (annotorious.events.ui.hasMouse && !keepShape)
    goog.style.showElement(this._editCanvas, false);

  this._currentSelector.stopSelection(keepShape);
